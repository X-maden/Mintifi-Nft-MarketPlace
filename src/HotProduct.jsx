...
// Move product data outside component
const HOT_PRODUCTS = [
  {
    id: 1,
...
export default function HotProductsSection({ boughtIds = [] }) {
  ...
  const visibleProducts = useMemo(() => 
    HOT_PRODUCTS.filter(product => !boughtIds.includes(product.id)),
    [boughtIds]
  );
  ...
  // Handle add to cart with debounced notification
  ...
  const ProductCard = useCallback(({ product }) => (
    <div className="snap-start flex-none w-72 mr-4 group cursor-pointer">
